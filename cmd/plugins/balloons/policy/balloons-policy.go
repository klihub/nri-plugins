...
func (p *balloons) Setup(policyOptions *policy.BackendOptions) error {
	...
	if !ok {
		log.Fatal("failed to initialize %s policy: config of wrong type %T",
	...
	case cfgapi.AmountCPUSet:
		...
		if err != nil {
			log.Fatal("failed to parse available CPU cpuset '%s': %w", amount, err)
		}
		p.allowed = cset
	...
	case cfgapi.AmountCPUSet:
		cset, err := amount.ParseCPUSet()
		if err != nil {
			log.Fatal("failed to parse reserved CPU cpuset '%s': %v", amount, err)
		}
		p.reserved = p.allowed.Intersection(cset)
	case cfgapi.AmountQuantity:
		qty, err := amount.ParseQuantity()
		if err != nil {
			log.Fatal("failed to parse reserved CPU quantity '%s': %v", amount, err)
		}
