...
func (p *policy) getMemOfferByHints(pool Node, req Request) (*libmem.Offer, error, bool) {
	ctr := req.GetContainer()

	memType := req.MemoryType()
	if memType == memoryPreserve {
		return nil, fmt.Errorf("%s by hints: memoryPreserve requested", pool.Name()), false
	}
	...
	for provider, hint := range ctr.GetTopologyHints() {
		...
		if !from.And(m).Contains(m.Slice()...) {
			return nil, fmt.Errorf("%s by hints: %s of wrong type (allowed %s)", pool.Name(), m, mtyp), false
		}
	...
	if zone == libmem.NodeMask(0) {
		return nil, fmt.Errorf("%s by hints: no pod resource API hints", pool.Name()), false
	}

	if zoneType := p.memAllocator.ZoneType(zone); zoneType != mtyp {
		return nil, fmt.Errorf("%s by hints: no type %s", mtyp.Clear(zoneType.Slice()...)), false
	}
