...
func (hog *Hog) Run() {
	...
	if hog.retouch != 0 {
		tick = time.NewTicker(hog.retouch).C
	}
	...
	waitSignalOrTimer := func() {
		orTimer := map[bool]string{true: " or memory retouch timer"}[tick != nil]
		log.Infof("waiting for hog signal %d (%s)%s...", hog.signal, hog.signal, orTimer)
		...
		case _ = <-sigCh:
			...
			buf = final
		case _ = <-tick:
			log.Info("memory re-touch timer expired...")
		}
	}
...
func main() {
	var (
		...
		hogSig  = flag.String("signal", "usr1", "signal to start hogging memory")
		retouch = flag.Duration("retouch-timer", 0, "re-touch memory pages this often")
		err     error
	)
