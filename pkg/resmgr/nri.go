...
func (p *nriPlugin) syncNamesToContainers(containers []cache.Container) []cache.Container {
	old := p.byname
	p.byname = make(map[string]cache.Container)

	for _, ctr := range containers {
		p.mapNameToContainer(ctr)
	}
	unmapped := make([]cache.Container, 0, len(old))
	for _, ctr := range old {
		unmapped = append(unmapped, ctr)
	}
...
func (p *nriPlugin) CreateContainer(ctx context.Context, podSandbox *api.PodSandbox, container *api.Container) (adjust *api.ContainerAdjustment, updates []*api.ContainerUpdate, retErr error) {
	...
	if old, ok := p.unmapName(c.PrettyName()); ok {
		nri.Debug("%s: releasing stale instance %s", c.PrettyName(), old.GetID())
		if err := m.policy.ReleaseResources(old); err != nil {
			nri.Error("%s: failed to release stale instance %s", c.PrettyName(), old.GetID())
		} else {
			nri.Info("%s: released stale instance %s", c.PrettyName(), old.GetID())
		}
