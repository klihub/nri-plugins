...
func TestBasicAllocation(t *testing.T) {
	...
	for _, tc := range []*testCase{
		...
		{
			name:   "6500m shared CPU from node #0",
			id:     "1",
			pool:   "node#0",
			shared: 6500,
		...
		{
			name:   "7500m shared CPU from node #0",
			id:     "2",
			pool:   "node#0",
			shared: 7500,
			common: pools["node#0"].Difference(isolated["node#0"]),
		...
		{
			name:   "8500m shared CPU from node #0",
			id:     "3",
			pool:   "node#0",
			shared: 8500,
			common: pools["node#0"].Difference(isolated["node#0"]),
		...
		{
			name:   "3500m shared CPU from node #0",
			id:     "4",
			pool:   "node#0",
			shared: 3500,
			common: pools["node#0"].Difference(isolated["node#0"]),
		...
		{
			name:   "26500m shared CPU from socket #1",
			id:     "6",
			pool:   "socket#1",
			shared: 26500,
		...
		{
			name:   "8500m shared CPU from node #1",
			id:     "7",
			pool:   "node#1",
			shared: 8500,
		...
		{
			name:   "7500m shared CPU from node #1",
			id:     "8",
			pool:   "node#1",
			shared: 7500,
			common: pools["node#1"].Difference(isolated["node#1"]),
		...
		{
			name:   "6500m shared CPU from node #1",
			id:     "9",
			pool:   "node#1",
			shared: 6500,
			common: pools["node#1"].Difference(isolated["node#1"]),
		...
		{
			name:   "3500m shared CPU from node #1",
			id:     "10",
			pool:   "node#1",
			shared: 3500,
			fail:   true,
		...
		{
			name:   "3000m shared CPU from node #1",
			id:     "10",
			pool:   "node#1",
			shared: 3000,
			common: pools["node#1"].Difference(isolated["node#1"]),
		...
		{
			name:    "26500m shared CPU from socket #1 and 3000m from node #1",
			id:      "",
			release: []string{"6", "10"},
		},
