...
// the request.
func Container(id, name string, limit int64, qos string, affin NodeMask) *Request {
	opts := []RequestOption{
		WithName(name),
		WithQosClass(qos),
	}
...
func ContainerWithTypes(id, name string, limit int64, qos string, affin NodeMask, types TypeMask) *Request {
	opts := []RequestOption{
		WithName(name),
		WithQosClass(qos),
...
func ContainerWithStrictTypes(id, name string, limit int64, qos string, affin NodeMask, types TypeMask) *Request {
	opts := []RequestOption{
		WithName(name),
		WithQosClass(qos),
...
// ReserveMemory is a convenience function to create an allocation request
// for reserving memory from the allocator. Once memory has been succesfully
// reserved, it is never moved by the allocator. ReserveMemory can be used
