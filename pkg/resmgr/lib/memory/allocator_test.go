...
func TestOffer(t *testing.T) {
	...
	o1, err := a.GetOffer(Container("id1", "test", 1, "burstable", NewNodeMask(0)))
	require.Nil(t, err, "unexpected GetOffer() error")
	require.NotNil(t, o1, "unexpected nil offer")

	o2, err := a.GetOffer(Container("id2", "test", 1, "burstable", NewNodeMask(1)))
	require.Nil(t, err, "unexpected GetOffer() error")
	...
	for _, c := range []*CustomFunctions{nil, customDefault} {
		...
		for _, tc := range []*testCase{
			...
			t.Run(name, func(t *testing.T) {
				...
				case tc.strict:
					req = ContainerWithStrictTypes(tc.id, name, tc.limit, qos, tc.affinity, tc.types)
				default:
					req = ContainerWithTypes(tc.id, name, tc.limit, qos, tc.affinity, tc.types)
				}
...
func TestStrictAllocation(t *testing.T) {
	...
	for _, tc := range []*testCase{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.strict {
				req = ContainerWithStrictTypes(tc.id, tc.name, tc.limit, tc.qosClass, tc.affinity, tc.types)
			} else {
				req = ContainerWithTypes(tc.id, tc.name, tc.limit, tc.qosClass, tc.affinity, tc.types)
			}
	...
	for _, tc := range []*testCase{
		...
		t.Run(tc.name, func(t *testing.T) {
			nodes, updates, err := a.Allocate(
				ContainerWithTypes(tc.id, tc.name, tc.limit, tc.qosClass, tc.affinity, tc.types),
			)
