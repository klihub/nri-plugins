...
func (m *resmgr) updateConfig(newCfg interface{}) error {
	...
	if !m.running {
		m.Infof("acquired initial configuration %s (generation %d):", meta.Name, meta.Generation)
	...
	m.Infof("configuration update %s (generation %d):", meta.Name, meta.Generation)
...
func (m *resmgr) reconfigure(cfg cfgapi.ResmgrConfig) error {
	...
	m.Errorf("reverting configuration, failed to apply update: %v", err)

	err = apply(m.cfg)
	if err != nil {
		m.Warnf("failed to revert configuration: %v", err)
	}
