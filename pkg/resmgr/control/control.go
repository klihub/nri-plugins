...
type Controller interface {
	...
	Start(cache.Cache, *cfgapi.Config) error
...
func (c *control) StartStopControllers(cfg *cfgapi.Config) error {
	...
	for name, controller := range controllers {
		if controller.name != cache.E2ETest {
			controller.enabled = enabled[name]
		} else {
			controller.enabled = enableTestAPIs
		}
	}
	c.cfg = cfg.DeepCopy()
	...
	for _, controller := range c.controllers {
		if controller.running {
			log.Info("stopping controller %s", controller.name)
			controller.c.Stop()
		}
	}
	...
	for _, controller := range c.controllers {
		if controller.enabled {
			log.Info("starting controller %s", controller.name)
			err := controller.c.Start(c.cache, cfg.DeepCopy())
			if err != nil {
