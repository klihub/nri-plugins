...
func (a *Agent) setupNodeWatch() error {
	...
	w, err := watch.New(context.Background(), "", a.nodeName,
...
func (a *Agent) setupNodeConfigWatch() error {
	...
	if a.hasLocalConfig() {
		w, err := a.cfgIf.WatchFile(a.configFile)
		if err != nil {
	...
	w, err := watch.New(context.Background(), a.namespace, a.nodeConfigName(),
...
func (a *Agent) setupGroupConfigWatch(group string) error {
	...
	w, err := watch.New(context.Background(), a.namespace, a.groupConfigName(),
		func(ctx context.Context, ns, name string) (watch.Interface, error) {
			return a.cfgIf.CreateWatch(ctx, ns, name)
		},
	)
...
func init() {
	...
	flag.StringVar(&defaultGroupLabel, "config-group-label", kubernetes.ResmgrKey("group"),
