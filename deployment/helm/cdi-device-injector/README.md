# CDI Device Injector Plugin

This chart deploys the CDI Device Injector NRI plugin. This plugin provides
a controlled mechansim for injecting CDI devices into containers.
...
- Container runtime:
  - containerD:
    ...
    - Enable NRI feature by following
      ...
      ```sh
      helm install my-cdi-injector nri-plugins/cdi-device-injector --set nri.runtime.patchConfig=true --namespace kube-system
      ```
  ...
  - CRI-O
    ...
    - Enable NRI feature by following
      ...
      ```sh
      helm install my-cdi-injector nri-plugins/nri-cdi-device-injector --set nri.runtime.patchConfig=true --namespace kube-system
      ```

## Installing the Chart

Path to the chart: `nri-cdi-device-injector`.

```sh
helm repo add nri-plugins https://containers.github.io/nri-plugins
helm install my-cdi-injector nri-plugins/nri-cdi-device-injector --namespace kube-system
```

The command above deploys cdi-device-injector NRI plugin on the Kubernetes cluster
...
```sh
# Install the cdi-device-injector plugin with custom values provided using the --set option
helm install my-cdi-injector nri-plugins/nri-cdi-device-injector --namespace kube-system --set nri.runtime.patchConfig=true
```

```sh
# Install the cdi-device-injector plugin with custom values specified in a custom values.yaml file
...
helm install my-cdi-injector nri-plugins/nri-cdi-device-injector --namespace kube-system -f myPath/values.yaml
```

## Uninstalling the Chart

To uninstall the cdi-device-injector plugin run the following command:

```sh
helm delete my-cdi-injector --namespace kube-system
```
...
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `image.name`             | [ghcr.io/containers/nri-plugins/cdi-device-injector](https://ghcr.io/containers/nri-plugins/cdi-device-injector)                        | container image name                                 |
